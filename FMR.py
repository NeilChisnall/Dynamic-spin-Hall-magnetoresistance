''' 
    ...
    of the magnetic layer. This incorporates STT from the incident spin 
    current and also spin pumping induced by magnetisation precession.
    Thermal effects are also accounted for.
'''
...
def time_evolution(m_init, t_span, t_points, t_N, n_points, jc_in_max, f_j, f_H):
    ...
    # analysis: takes one oscillation, takes the rms voltage 
    jc_long_array[t_points-1] = jc_long_array[t_points-2]
    jc_trans_array[t_points-1] = jc_trans_array[t_points-2]
    V_array[:,t_points-1] = V_array[:,t_points-2]
    period = 1 / f_H
    V_points = int(np.round(period / dt))
    start_rms_index = t_points - 1 - V_points
    V_slice = V_array[:,start_rms_index:]
    ts_slice = ts[start_rms_index:]
    V_rms_long = np.sqrt(np.sum( (V_slice[0,:])**2 ))
    V_rms_trans = np.sqrt(np.sum( (V_slice[1,:])**2 ))
    return ts, ms, V_rms_long, V_rms_trans


if __name__ == "__main__":

    m_init = np.array([0.985, 00.169, 0])
    m_init = m_init / np.linalg.norm(m_init)
    t_step = 6e-13
    t_span = 1.5e-9
    t_points = np.int(np.round(t_span / t_step))
    FIELDS = np.linspace(0.495, 0.505,15)
    f_H = 1.3e10
    f_j = 0
    n_points = 30
    jc_in_max = 0

    V_long = np.zeros(np.shape(FIELDS))
    V_trans = np.zeros(np.shape(FIELDS))

    for l in progressbar.progressbar(range(len(FIELDS))):

        ts, ms, V_long[l], V_trans[l]  = time_evolution(m_init, t_span, t_points, t_N, n_points, jc_in_max, f_j, f_H)
    
    # data = np.column_stack((FREQUENCIES, V_long, V_trans))
    # description = str("KittelCurve_")
    # np.savetxt( str(description + str(FIELDS[l]) + '.csv'), data, delimiter=',', header="frequencies, V_long, V_trans")
